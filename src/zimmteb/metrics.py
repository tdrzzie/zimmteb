...
def ranking_metrics(positives: list[str], scores: dict[str, float]) -> dict[str, float]:
    ...
    evaluator = pytrec_eval.RelevanceEvaluator(
        judgments, {"recall.1,5,10,20", "ndcg_cut.10", "map_cut.10"}
    )
    ...
    reciprocal = next(
        (1 / rank for rank, id_ in enumerate(ranking[:10], 1) if id_ in positives), 0.0
    )
    return {
        **{f"recall_at_{k}": raw[f"recall_{k}"] for k in (1, 5, 10, 20)},
        "mrr_at_10": reciprocal,
        "ndcg_at_10": raw["ndcg_cut_10"],
        "map_at_10": raw["map_cut_10"],
    }
...
def slices(queries: list[QueryResult]) -> list[SliceResult]:
    ...
    return [
        SliceResult(
            ...
            count=len(items),
            metrics={
                metric: float(np.mean([q.metrics[metric] for q in items])) for metric in METRICS
            },
        )
...
def paired_bootstrap(
    left: list[float], right: list[float], seed: int = 42, repetitions: int = 2000
) -> dict[str, float]:
    ...
    draws = np.array(
        [rng.choice(difference, len(difference), replace=True).mean() for _ in range(repetitions)]
    )
    low, high = np.quantile(draws, [0.025, 0.975])
    return {
        "delta_right_minus_left": float(difference.mean()),
        "ci95_low": float(low),
