...
def markdown_report(result: RunResult) -> str:
    lines = ["# ZimMTEB retrieval report", "", f"Run: `{result.run_id}`", "",
             ...
             "**Synthetic infrastructure measurements only. No linguistic validity or model superiority is established.**" if result.synthetic else "Review dataset governance before interpreting scores.", "",
    ...
    for item in sorted(result.slices, key=lambda s: (order.get(s.dimension, 4), s.dimension, s.value)):
    ...
    for dimension, kind in (("query_language", "languages"), ("domain", "domains")):
        ...
        if missing:
            lines.extend(["", f"Not evaluated ({dimension}): {', '.join(missing)}. These are missing values, not zero scores."])
    lines.extend(["", "## Efficiency", "", "Indexing and query phases are measured separately. Cached runs measure cache reads, not inference speed.", "Batch query timing is not single-request latency. RSS peak is sampled; CUDA peak is allocator memory.", "", "```json", json.dumps(result.efficiency, indent=2), "```", "", "## Failures", ""])
    failed = [q for q in result.queries if q.failures]
    lines.extend(f"- `{q.query_id}` ({q.query_language}, {q.domain}): {', '.join(q.failures)}; top result `{q.ranking[0]}`" for q in failed)
    if not failed:
        lines.append("No automatically detected top-1 failures on this fixture.")
    lines.extend(["", "These labels are mechanical ranking observations; lexical traps and semantic causes require human analysis.", "", "## Reproducibility", "", f"Git commit: `{result.environment.get('git_commit')}`; dirty: `{result.environment.get('git_dirty')}`.", f"Seed: {result.config.seed}; review level: {result.human_review_status}.", "Native MTEB results and the manifest accompany this report. No task or score was submitted upstream.", "", "## Limitations", "", *[f"- {warning}" for warning in result.warnings], "- This dataset is too small and unreviewed for significance claims.", "- No trained zim-embed-small or zim-reranker is supplied."])
    return "\n".join(lines) + "\n"
...
def compare(left: RunResult, right: RunResult) -> dict[str, object]:
    for field in ("benchmark_version", "dataset_id", "dataset_version", "dataset_checksum", "evaluation_engine"):
    ...
    ids = sorted(a)
    return {"metric": "ndcg_at_10", "queries": len(ids),
            **paired_bootstrap([a[id_].metrics["ndcg_at_10"] for id_ in ids], [b[id_].metrics["ndcg_at_10"] for id_ in ids]),
