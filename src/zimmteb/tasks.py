...
class ZimRetrievalTask(AbsTaskRetrieval):
    def __init__(self, data: RetrievalDataset, queries: list[Query], split: str = "test") -> None:
        ...
        self.metadata = TaskMetadata(
            name="ZimMTEBTinyRetrieval", description="Synthetic unreviewed Zimbabwe-language infrastructure fixture; not a scientific benchmark.",
            dataset={"path": "zimmteb/local-synthetic", "revision": data.actual_checksum},
            type="Retrieval", category="t2t", eval_splits=[split],
            eval_langs={"default": [f"{code}-Latn" for code in sorted(codes)]},
            main_score="ndcg_at_10", license="cc0-1.0", domains=["Constructed"],
            annotations_creators="LM-generated", is_public=False,
        )
        super().__init__()
    ...
    def load_data(self, **kwargs: Any) -> None:
        self.dataset = {"default": {self.selected_split: {
            "corpus": Dataset.from_list([{"id": d.document_id, "text": d.text, "title": d.title} for d in self.source.documents]),
            "queries": Dataset.from_list([{"id": q.id, "text": q.query} for q in self.selected_queries]),
            "relevant_docs": {q.id: {id_: 1 for id_ in q.positive_document_ids} for q in self.selected_queries},
            "top_ranked": None,
        }}}
...
def local_benchmark(task: ZimRetrievalTask) -> Benchmark:
    return Benchmark(name="ZimMTEB(v0.1,synth)", tasks=[task],
...
class MTEBSearchBridge:
    ...
    def __init__(self, adapter: EmbeddingModelAdapter, cache: Path | None, identity: dict[str, Any], device: str) -> None:
        ...
        self.mteb_model_meta = ModelMeta.create_empty(overwrites={
            "name": adapter.config.model_name, "revision": adapter.config.revision,
            "framework": ["Sentence Transformers"] if adapter.config.adapter == "sentence-transformers" else [],
            "n_parameters": meta.get("parameter_count"), "embed_dim": meta.get("embedding_dimension"),
            "max_tokens": adapter.config.max_length, "similarity_fn_name": "dot",
        })
        self.corpus_ids: list[str] = []
        self.corpus_embeddings: Embeddings = np.empty((0, 0), dtype=np.float32)
        self.rankings: dict[str, dict[str, float]] = {}
        self.efficiency: dict[str, Any] = {"cache_hits": [], "precision": adapter.config.precision,
    ...
    def _encode(self, texts: list[str], role: str, split: str) -> Embeddings:
        key = content_key({**self.identity, "model": self.adapter.config.model_dump(), "role": role,
                           "split": split, "texts": texts, "preprocessing": "title-space-text-v1"})
        values = self.cache.get(key, len(texts)) if self.cache else None
        if values is not None:
            self.efficiency["cache_hits"].append(role)
            return values
        values = self.adapter.encode_queries(texts) if role == "query" else self.adapter.encode_documents(texts)
    ...
    def index(self, corpus: Any, *, hf_split: str, **kwargs: Any) -> None:
        ...
        self.efficiency.update(index_seconds=elapsed, documents_per_second=len(texts) / elapsed,
                               corpus_documents=len(texts), embedding_dimension=self.corpus_embeddings.shape[1])

    def search(self, queries: Any, *, hf_split: str, top_k: int, top_ranked: Any = None, **kwargs: Any) -> dict[str, dict[str, float]]:
        ...
        for offset in range(0, len(embeddings), self.adapter.config.batch_size):
            block = embeddings[offset:offset + self.adapter.config.batch_size] @ self.corpus_embeddings.T
            for id_, row in zip(query_ids[offset:offset + len(block)], block, strict=True):
                if not np.isfinite(row).all():
                    raise ValueError("Non-finite similarity scores")
                order = sorted(range(len(row)), key=lambda i: (float(row[i]), self.corpus_ids[i]), reverse=True)[:top_k]
                self.rankings[id_] = {self.corpus_ids[i]: float(row[i]) for i in order}
        elapsed = time.perf_counter() - start
        self.efficiency.update(query_encoding_seconds=encoded - start,
                               query_search_seconds=elapsed, queries_per_second=len(query_ids) / elapsed,
